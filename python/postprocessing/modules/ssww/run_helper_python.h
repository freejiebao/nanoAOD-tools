...
RVec<bool> lepton_real_code(int nlepton, rvec_i lepton_pdg_id, rvec_f lepton_pt, rvec_f lepton_eta, int nGenPart, rvec_i GenPart_pdgId, rvec_f GenPart_pt, rvec_i GenPart_statusFlags, rvec_f GenPart_eta, rvec_f GenPart_phi)
{
   RVec<bool> lepton_real_new(nlepton);
   for(int i=0; i<nlepton; i++){
    lepton_real_new[i]=false;
    for(int j=0; j<nGenPart; j++){
        if(GenPart_pt[j]>5 && abs(GenPart_pdgId[j])==abs(lepton_pdg_id[i]) && (GenPart_statusFlags==0 || GenPart_statusFlags==3) && (sqrt(pow(lepton_eta[i] - GenPart_eta[j], 2) + pow(abs(abs(lepton_phi[i] - GenPart_phi[j])-TMath::Pi())-TMath::Pi(), 2)) < 0.3)){
            lepton_real_new[i]=true;
            break;
        }
    }
   }
   return lepton_real_new;
}
