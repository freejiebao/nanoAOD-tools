    ...
    for imc in mc_chain:
        for i in range(0,len(samples[imc])):
            ...
            if args.fixreal:
                ...
                df01=df.Filter("nGenJet>0").Define("lepton_real_new","bool lepton_real_new[nlepton];for(int i=0; i<nlepton; i++){for(int j=0; j<nGenJet; j++){if(GenPart_pt[j]>5 && abs(GenPart_pdgId[j])==abs(lepton_pdg_id[i]) && (GenPart_statusFlags==0 || GenPart_statusFlags==6) && (sqrt(pow(lepton_eta[i] - GenPart_pt_eta[j], 2) + pow(abs(abs(lepton_phi[i] - GenPart_phi[j])-pi)-pi, 2)) < 0.3)){lepton_real_new[i]=True;} else{lepton_real_new[i]=False;};}}; return lepton_real_new;")
