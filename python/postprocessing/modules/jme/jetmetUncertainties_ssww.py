...
class jetmetUncertaintiesProducer(Module):
    def __init__(self, era, globalTag, jesUncertainties = [ "Total" ], jetType = "AK4PFchs", redoJEC=False, noGroom=False, jerTag="", jmrVals = [], jmsVals = []):
        ...
        if "AK4" in jetType :
            ...
            if not self.noGroom:
                self.doGroomed = True
                self.puppiCorrFile = ROOT.TFile.Open(os.environ['CMSSW_BASE'] + "/python/PhysicsTools/NanoAODTools/postprocessing/data/jme/puppiCorr.root")
        ...
        self.jesInputArchivePath = os.environ['CMSSW_BASE'] + "/python/PhysicsTools/NanoAODTools/postprocessing/data/jme/"
    ...
    def analyze(self, event):
        ...
        for jet in jets:
            ...
            if self.redoJEC :
                (jet_pt, jet_mass) = self.jetReCalibrator.correct(Jets[jet.idx],rho)
                jet.pt = jet_pt
                jet.mass = jet_mass
